...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/georgswift1`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/georgswift1` directory. You need to make sure you change the `repository` variable in your `chart/georgswift1/values.yaml` file points to the docker image containing your runnable application.
